---
title: "ESP-IDF Series: GPIO get started"
date: "2026-01-08"
# If default Espressif author is needed, uncomment this
# showAuthor: true
# Add a summary
summary: "This article explains how to configure and control GPIO pins on Espressif microcontrollers, covering push-pull and open-drain output modes, drive capability, and library usage. It demonstrates a hands-on example of blinking an LED using `gpio_set_direction` and `gpio_set_level`." 
...
Modern microcontrollers offer several ways to configure an output GPIO. We will discuss the two main options:
...
When you drive a pin HIGH or LOW, and the pin connected to a load, you must supply enough current to maintain the desired logic level.
...
Instructions for including a library are always found in the [ESP-IDF Programming Guide](https://docs.espressif.com/), under the **API Reference** section of the corresponding peripheral page.

The [GPIO API reference](https://docs.espressif.com/projects/esp-idf/en/v5.5.2/esp32c61/api-reference/peripherals/gpio.html#api-reference-normal-gpio) shows that the GPIO peripheral is managed through the `driver/gpio.h` library, which must be included as:

```c
#include "driver/gpio.h"
```

If you are using it inside a component other than `main` (which by default already depends on all ESP-IDF components), you must add one of the following to its `CMakeLists.txt`:
...
You can verify this in the [pin overview](https://documentation.espressif.com/esp32-c61_datasheet_en.html#[15,%22XYZ%22,56.69,785.2,null]) section of the datasheet.
...
Most modern Espressif EVKs do not have a simple onboard LED (also called a GPIO LED) and typically feature RGB LEDs instead. For this reason, we need to connect an external LED to a GPIO. We will use GPIO5. 
...
In the schematic, a 100 Ohm resistor is chosen based on a LED forward voltage of 2.7V and a forward current of 6mA. If you use a LED with different specifications, adjust the resistor value accordingly.
{{< /alert >}}
...
For this simple application, we only need to set GPIO5 as an output. To keep things organized, we will define its value after the includes. In the same place, we also define the TAG string for logging.
...
and in the `app_main` function we can configure it as:
...
The [`vTaskDelay`](https://docs.espressif.com/projects/esp-techpedia/en/latest/esp-friends/get-started/basic-concepts/common-freertos-api/task-control.html#vtaskdelay) function takes the number of [ticks](https://www.freertos.org/Documentation/02-Kernel/05-RTOS-implementation-tutorial/02-Building-blocks/03-The-RTOS-tick) as its argument. To work with milliseconds instead of ticks, we can use the `pdMS_TO_TICKS` macro, which converts a given millisecond value to the corresponding number of ticks.

{{< /alert >}}

<details>
<summary>Hands-on code</summary>
...
In this article, we demonstrated how to configure and control an output GPIO on Espressif microcontrollers, set up a blinking LED, and use FreeRTOS for timing, providing a practical introduction to GPIO programming. 
